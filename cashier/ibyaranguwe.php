...
echo"<option value='$name'>$name</option>";	 
  ...
  <tr><td> <b>Quantity</b> </td><td> <input type="number" name="qty"step="any" ></td></tr>
   <tr><td> <b>Ayo Waranguje</b> </td><td> <input type="number"  step="any"  name="org_price" ></td></tr>
...
if(isset($_POST['update'])){
	...
	while($row_pro= mysqli_fetch_array($run_select)){
		...
		$ayarangujwe=$quantity*$o_price;
		
	  //  $azavamo=$quantity*$price;
		
		$total_qty=$quantity+$qty;
		// ayarangujwe kuri new stock
		$ayarangujwe=$total_qty*$ayaranguweho;
	...
	values('$pro_name','$date','LIVRAISON','$quantity','$ayaranguweho','$ayarangujwe')";
	...
	if($run_upd){
		...
		echo"<script>window.open('index.php?ibyaranguwe','_self')</script>";
	}
